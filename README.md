...
- **对话与划词解释**：聊天面板发送消息；在 PDF 中划词可触发解释（对接 `/api/explain`，携带当前页上下文并优先基于当前论文 RAG），并返回结构统一的 `rag_sources` 依据片段。
...
- Python 结合当前论文 `pdfId`、篇章结构、Chroma RAG 片段和 LLM 返回前置概念图谱、学习路径、补课清单和结构统一的 RAG 依据片段。
