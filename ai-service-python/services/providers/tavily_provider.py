...
class TavilyProviderError(RuntimeError):
    def __init__(self, code: str, status_code: Optional[int] = None):
        ...
        super().__init__(f"Tavily Search request failed ({code}).")
            ...
            try:
                ...
                if status_code in RETRYABLE_STATUS_CODES and attempt < MAX_RETRIES:
                    ...
                    if retry_delay is not None:
                        continue
                raise TavilyProviderError("http_error", status_code=status_code)
            except TavilyProviderError:
                raise
            except requests.Timeout:
                raise TavilyProviderError("timeout") from None
            except requests.RequestException:
                raise TavilyProviderError("network_error") from None
            finally:
                if response is not None:
                    response.close()

        raise TavilyProviderError("http_error")
...
def build_tavily_provider(config: Any) -> TavilyProvider:
    api_key = None
    if isinstance(config, dict) or hasattr(config, "get"):
        env_key = str(config.get(TAVILY_API_KEY_ENV, "")).strip()
        if env_key:
            api_key = env_key
    if not api_key:
        raise ValueError("Tavily Search API key is required.")
...
def _validate_query(query: Any) -> str:
    if not isinstance(query, str) or not query.strip():
        raise ValueError("Tavily Search query must be a non-empty string.")
    return _bounded_text(query, MAX_QUERY_CHARS)


def _validate_limit(limit: Any) -> int:
    if isinstance(limit, bool) or not isinstance(limit, int) or not 1 <= limit <= MAX_RESULTS:
        raise ValueError("Tavily Search result limit must be an integer between 1 and 20.")
    return limit
...
def _read_json_response(response: Any) -> Dict[str, Any]:
    ...
    if content_length:
        try:
            if int(content_length) > MAX_RESPONSE_BYTES:
                raise TavilyProviderError("response_too_large")
        except (TypeError, ValueError):
            raise TavilyProviderError("invalid_response") from None
    ...
    if not isinstance(payload, dict):
        raise TavilyProviderError("invalid_response")
    return payload


def _extract_answer(payload: Dict[str, Any]) -> Optional[str]:
    answer = payload.get("answer")
    if isinstance(answer, str) and answer.strip():
        return _bounded_text(answer, MAX_ANSWER_CHARS)
    return None


def _build_answer_license(answer_text: Optional[str]) -> str:
    if not answer_text:
        return ""
    return json.dumps({
...
def _normalize_items(
    ...
    answer_license = _build_answer_license(answer_text)
    results = []
    for raw in results_list:
        if not isinstance(raw, dict):
            continue
        mapped = _map_tavily_result(raw, query=query, retrieved_at=retrieved_at, license_value=answer_license)
...
def _map_tavily_result(item: Dict[str, Any], query: str, retrieved_at: str, license_value: str) -> Dict[str, Any]:
    return {
        ...
        "authors": [],
        "year": None,
        "abstract": _sanitize_untrusted_metadata(item.get("content"), MAX_CONTENT_CHARS),
        "doi": "",
