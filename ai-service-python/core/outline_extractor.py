...
def _rate_pdf_heading_likelihood(
    ...
    # Font size boost: headings are bigger.
    ...
    # Short text is more heading-like.
    words = text.split()
    if len(words) <= 8:
        score += 0.08
    elif len(words) <= 14:
    ...
    # CJK text with 2-10 characters is very heading-like.
    cjk_chars = len(re.findall(r"[一-鿿]", text))
    if cjk_chars >= 2 and cjk_chars <= 20:
        score += 0.06

    # Penalty: line that looks like body text.
