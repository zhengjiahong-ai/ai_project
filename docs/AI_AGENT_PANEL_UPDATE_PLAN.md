...
- 项目级任务历史接口已经补齐，当前可通过 `GET /api/agent-projects/{projectId}/tasks` 按更新时间倒序恢复完整任务快照。
- 当前 `agent_orchestrator.py` 仍是轻量规则型执行策略，不是 LangGraph/Deep Orchestrator。
- 当前事件流是任务快照中的摘要事件，并已随 Agent SQLite 快照恢复；它仍不是完整的底层工具级可回放事件日志。

换句话说，AI 层已经搭出可用的第一版房子，项目/任务持久化和项目级任务历史恢复地基已经补齐，后续还需要更强编排策略。
...
Python SQLite 已不只存 `research_tasks`，还新增了：
...
- 服务重启后恢复项目、latest task 和终态任务输出
- 最近任务恢复
- 面向前端时间线恢复事件摘要
- trace 和事件分离

重启前仍处于 `running` 或 `pending` 的 Agent 任务会恢复为 `failed`，并记录 `task_expired` 事件，避免前端误判任务仍在执行。
...
Agent 任务已从创建即执行改为 Planner 审查、Executor、Aggregator 审查三段式流程；计划和终稿审查结果随任务历史恢复。
