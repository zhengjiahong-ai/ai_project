...
| POST | `/api/chat` | `{ "message": string, "pdfId": any }` | 对话，Java 转发到 Python；**当前 Python 未实现 `/api/chat`，需在 Python 中补全或前端暂时 Mock** |
| GET | `/api/chat/history/:sessionId` | - | 对话历史；**后端未实现，前端已预留** |
| POST | `/api/critical-reading/:pdfId` | - | 批判性阅读；**后端未实现，前端已预留** |

- **说明**：  
  - Java `AcademicController` 中 `explainTerm` 接收 `Map<String, String>`，若前端传 `text/pdfId/pageNumber`，需在 Java 层做字段映射或适配为 Python 所需的 `term`、`context`。  
...
| POST chat → 转发 | POST `/api/chat` | **Python 未实现**，需在 `main.py` 中新增或暂时返回占位 |
